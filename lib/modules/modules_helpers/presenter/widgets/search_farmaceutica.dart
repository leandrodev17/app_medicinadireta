...
class SearchFarmaceutica extends BasePage {
  SearchFarmaceutica({Key? key}) : super(key: key);

  @override
  SearchFarmaceuticaState createState() => SearchFarmaceuticaState();
}

class SearchFarmaceuticaState extends BaseState<SearchFarmaceutica> with WidgetsBindingObserver, BasicPage {
  final _controller = Get.find<MedicalController>();
  final _scrollController = ScrollController();
  final _debouncer = Debouncer(delay: const Duration(milliseconds: 300));

  @override
  String screenName() => 'Forma Farmacêutica'.tr;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _controller.listFormaFarmaceutica.clear();
    });
        ...
        slivers: [
          SliverToBoxAdapter(
            child: AppTextField(
              ...
              onChanged: (dynamic d) {
                _debouncer(() async {
                  if (d != '' && d.length > 3) {
                    await _controller.searchFormaFarmaceutica(d.toString().toLowerCase());
                  } else {
                    _debouncer.cancel();
                    _controller.isLoadingSearchFormaFarmaceutica.value = AppLoadingStatus.notLoading;
                  }
                });
              },
            ),
          ...
          Obx(() {
            return SliverToBoxAdapter(
              child: Visibility(
                visible: _controller.isLoadingSearchFormaFarmaceutica.value == AppLoadingStatus.notConnecting,
                child: Container(
          ...
          Obx(() {
            if ((_controller.isLoadingSearchFormaFarmaceutica.value == AppLoadingStatus.notLoading) ||
                (_controller.isLoadingSearchFormaFarmaceutica.value == AppLoadingStatus.nextPageLoading)) {
              if (_controller.listFormaFarmaceutica.isEmpty) {
                return SliverToBoxAdapter(
                  child: Container(
                    height: view.localWidgetSize?.height,
                    alignment: Alignment.center,
              ...
              return SliverList(
                delegate: SliverChildBuilderDelegate(
                  (context, index) {
                    final FormaFarmaceutica model = _controller.listFormaFarmaceutica[index];
                    String? medDesc = model.descricao?.toString() ?? 'not_value'.tr;
                    return Container(
                      ...
                      child: ListTile(
                        ...
                        onTap: () => _controller.setFormaFarmaceuticaForm(model),
                      ),
                    );
                  },
                  childCount: _controller.listFormaFarmaceutica.length,
                ),
              );
            } else {
              if (_controller.isLoadingSearchFormaFarmaceutica.value == AppLoadingStatus.shimmerLoading) {
