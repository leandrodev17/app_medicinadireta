...
import 'package:medicinadireta/modules/patient_information/submodules/form_plan/presenter/widgets/card_item_form_plan.dart';
import 'package:medicinadireta/modules/patient_information/submodules/form_plan/presenter/widgets/shimmer_form_plan.dart';
...
class FormPlanPhone extends BasePage {
  FormPlanPhone({Key? key}) : super(key: key);

  @override
  FormPlanPhoneState createState() => FormPlanPhoneState();
}

class FormPlanPhoneState extends BaseState<FormPlanPhone> with WidgetsBindingObserver, BasicPage {
  final _controller = Get.find<FormPlanController>();
  final _scrollController = ScrollController();
  final ParamsToNavigationPage _params = Get.arguments;

  @override
  String screenName() => 'Formulários'.tr;
          ...
          slivers: [
            ...
            Obx(() {
              if ((_controller.isLoading.value == AppLoadingStatus.notLoading) || (_controller.isLoading.value == AppLoadingStatus.nextPageLoading)) {
                if (_controller.listFormPlan.isEmpty) {
                  return SliverToBoxAdapter(
                    child: Container(
                      height: view.localWidgetSize?.height,
                      alignment: Alignment.center,
                ...
                return SliverList(
                  delegate: SliverChildBuilderDelegate(
                    (context, index) {
                      final FormPlanModel model = _controller.listFormPlan[index];
                      return CardItemFormPlan(
                        controller: _controller,
                        itemIndex: index,
                        model: model,
                    ...
                    childCount: _controller.listFormPlan.length,
                  ),
                );
              } else {
                if (_controller.isLoading.value == AppLoadingStatus.shimmerLoading) {
                  return const SliverToBoxAdapter(child: ShimmerFormPlan());
                } else {
