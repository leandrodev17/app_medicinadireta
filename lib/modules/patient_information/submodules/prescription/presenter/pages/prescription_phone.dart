...
class PrescriptionPhone extends BasePage {
  PrescriptionPhone({Key? key}) : super(key: key);

  @override
  PrescriptionPhoneState createState() => PrescriptionPhoneState();
}

class PrescriptionPhoneState extends BaseState<PrescriptionPhone> with WidgetsBindingObserver, BasicPage {
  final _controller = Get.find<PrescriptionController>();
  final _scrollController = ScrollController();
  final ParamsToNavigationPage _params = Get.arguments;

  @override
  String screenName() => 'Receitas'.tr;
          ...
          slivers: [
            ...
            Obx(() {
              if ((_controller.isLoading.value == AppLoadingStatus.notLoading) || (_controller.isLoading.value == AppLoadingStatus.nextPageLoading)) {
                if (_controller.listPrescription.isEmpty) {
                  return SliverToBoxAdapter(
                    child: Container(
                      height: view.localWidgetSize?.height,
                      alignment: Alignment.center,
                ...
                return SliverList(
                  delegate: SliverChildBuilderDelegate(
                    (context, index) {
                      final PrescriptionModel model = _controller.listPrescription[index];
                      return CardItemPrescription(
                        controller: _controller,
                        itemIndex: index,
                        model: model,
                    ...
                    childCount: _controller.listPrescription.length,
                  ),
                );
              } else {
                if (_controller.isLoading.value == AppLoadingStatus.shimmerLoading) {
                  return const SliverToBoxAdapter(child: ShimmerPrescription());
                } else {
