...
import './properties/selection_list_view_props.dart';
...
class SelectionWidgetState<T> extends State<SelectionWidget<T>> {
                        ...
                        builder: (context, snapshot) {
                          if (snapshot.hasError) {
                            ...
                            if (widget.emptyBuilder != null)
                              ...
                              return const Center(
                                child: const Text("Nenhum dado encontrado, verifique!"),
                              );
                          }
  ...
  Widget _itemWidgetSingleSelection(T item) {
    return (widget.itemBuilder != null)
        ...
        : Container(
            decoration: BoxDecoration(border: Border(bottom: BorderSide(color: Color(0xFFB6B9D0), width: .6))),
            ...
            child: ListTile(
              ...
              title: Text(
                ...
                style: Theme.of(context).textTheme.subtitle1?.copyWith(fontSize: 11, fontWeight: FontWeight.w500),
              ),
  ...
  void selectItems(List<T> itemsToSelect) {
    ...
    itemsToSelect.forEach((i) {
      if (!_isSelectedItem(i) /*check if the item is already selected*/ && !_isDisabled(i) /*escape disabled items*/) {
        ...
        if (widget.popupOnItemAdded != null) widget.popupOnItemAdded!(_selectedItems, i);
      }
    });
  ...
  void deselectItems(List<T> itemsToDeselect) {
    ...
    itemsToDeselect.forEach((i) {
      ...
      if (index > -1) /*check if the item is already selected*/ {
        ...
        if (widget.popupOnItemRemoved != null) widget.popupOnItemRemoved!(_selectedItems, i);
      }
    });
    _selectedItemsNotifier.value = List.from(newSelectedItems);
  }
